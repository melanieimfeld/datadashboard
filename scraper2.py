...
def scrapeLinks(number):
	...
	#------------append 1st level----------------
		for i in range(number):
			mandate1 = getDateName(linksList[i])
			nodes.append({"id": mandate1[0],"keyword": word, "date":mandate1[1],"level": 1, "outDegree": len(mandate1[4])})
    	...
    	#problem: in edges, all ids are appended, in nodes only the ones with dates. 
        	...
        	#print("next edgelist", mandate2[3], mandate2[1],mandate2[0])
					oj = {"id": mandate2[0],"keyword": word, "date":mandate2[1], "level": 2, "outDegree":len(mandate2[4])}
					nodes.append(oj)
					linksLevel3.append([mandate2[0], mandate2[2]])
        	#print("mandates linked",len(mandate2[4]),"list",mandate2[4], "mandate", mandate2[0])
        	#edges + mandate2[3]
					edges.append({"source": mandate1[0], "target": mandate2[0]})
	...
	with open('nodes1.txt', 'w') as file:
		file.write(json.dumps(nodes)) # use `json.loads` to do the reverse

	with open('edges1.txt', 'w') as file:
...
def getDateName(link):
    ...
    #print(mRefsLinks)
    return mID, mDateParsed, mRefsLinks, subEdges, mRefsNames
